...
for /l %%i in (0,1,%SERVER_COUNT%) do (
    ...
    if defined SERVER_ARRAY[%%i] (
        ...
        if defined CHUNK_FILE (
            ...
            REM Use the helper batch script to run the client
            start "" /b call run_client.bat "!CHUNK!" "!OUTPUT_PREFIX!" "!SERVER!" "!WORKERS_PER_SERVER!" "!BATCH_SIZE!" > "%OUTPUT_DIR%\log_!CHUNK_FILE!.log" 2>&1
            
            REM Wait a moment for the process to start
            timeout /t 1 /nobreak > nul
            
            REM Store the process ID
            for /f "tokens=2" %%p in ('tasklist /fi "imagename eq cmd.exe" /nh') do (
